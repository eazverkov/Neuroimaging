...
class Domain2Var1Model():
    ...
    def get_x_y(self):
        y_train = self.train_scores[['Id', 'domain2_var1']]
        y_train = y_train.loc[ pd.isnull(y_train['domain2_var1'])==False , :].reset_index(drop=True)
        X_train = pd.merge(self.data, y_train, on='Id', how='right').drop(['domain2_var1'], axis=1).reset_index(drop=True)
    ...
    def cross_validation(self):
        ...
        for train_index, test_index in kf.split(X):
            ...
            y_train, y_test = y.loc[train_index, 'domain2_var1'], y.loc[test_index, 'domain2_var1']
  ...
  #  return test_prediction, mae, average_pred, metric
    ...
    def predict_for_test(self):
        ...
        model.fit(X.loc[ : , self.features],  y.loc[:, 'domain2_var1'])
...
if __name__ == "__main__":
    ...
    Domain2_var1_model.cross_validation()
    Domain2_var1_model.predict_for_test()
