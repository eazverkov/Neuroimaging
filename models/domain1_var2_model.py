...
class Domain1Var2Model():
    ...
    def get_x_y(self):
        y_train = self.train_scores[['Id', 'domain1_var2']]
        y_train = y_train.loc[ pd.isnull(y_train['domain1_var2'])==False , :].reset_index(drop=True)
        X_train = pd.merge(self.data, y_train, on='Id', how='right').drop(['domain1_var2'], axis=1).reset_index(drop=True)
    ...
    def cross_validation(self):
        ...
        for train_index, test_index in kf.split(X):
            ...
            y_train, y_test = y.loc[train_index, 'domain1_var2'], y.loc[test_index, 'domain1_var2']
  ...
  #  return test_prediction, mae, average_pred, metric
    ...
    def predict_for_test(self):
        ...
        model.fit(X.loc[ : , self.features],  y.loc[:, 'domain1_var2'])
...
if __name__ == "__main__":
    ...
    Domain1_var2_model.cross_validation()
    Domain1_var2_model.predict_for_test()
